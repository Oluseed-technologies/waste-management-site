...
const Dashboard = () => {
    const renderSummary = Summary.map((data, index) => {
        return (
            ...
            <p className='text-dark8 mt-5 font-medium' > <span className='text-primary-200 text-xl'> {data.percent} </span> since last month  </p>
            </div>
        )
    })
    const arr = new Array(6).fill(0)
    const renderPageView = arr.map((data, index) => {
        return <div className='flex m-5 justify-between items-center'> 
        <p>/Argon </p>
        <p>5000 </p>
        <p>319 </p>
        <p>46.513 </p>
    ...
    return (
        ...
        <button className='bg-primary text-white px-5 py-2 rounded-md'> SEE ALL</button>
        </div>    
      <div className='  '> 
